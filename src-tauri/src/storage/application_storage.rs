...
fn matches_filter(application: &Application, filter: &ApplicationFilter) -> bool {
    if let Some(ref status) = filter.status {
        // 注意：Application 结构体没有直接存储 status 字段
        // 这里根据需求，status 默认设为 applied
        // 实际项目中 status 应该存储在 frontmatter 中
        let _ = status;
    }
...
pub fn create_application(input: CreateApplicationInput) -> Result<Application, AppError> {
    ...
    let body = format!(
        "## JD 原文\n\n{}\n\n## 生成的打招呼\n\n{}",
        application.jd_content, application.greeting
    );
...
pub fn update_application(id: &str, input: UpdateApplicationInput) -> Result<Application, AppError> {
    ...
    let updated = Application {
        ...
        greeting: input.greeting.unwrap_or(existing.greeting),
        ..existing
    };
    ...
    let body = format!(
        "## JD 原文\n\n{}\n\n## 生成的打招呼\n\n{}",
        updated.jd_content, updated.greeting
    );
...
/// 更新投递状态
pub fn update_application_status(id: &str, _status: ApplicationStatus) -> Result<Application, AppError> {
    // 目前 status 字段未存储在 frontmatter 中（设计文档中投递记录是自动生成快照）
    // 如果将来需要存储 status，需要在 ApplicationFrontmatter 中添加 status 字段
    // 当前返回现有记录
    get_application(id)?
        .ok_or_else(|| AppError::NotFound(format!("投递记录不存在: {}", id)))
}
...
mod tests {
    ...
    fn test_create_and_list_application() {
        let input = CreateApplicationInput {
            ...
            jd_content: "招聘测试工程师...".to_string(),
            greeting: "您好，我有3年测试经验...".to_string(),
        };
